...
use mp4::{Mp4Writer, Mp4Config, TrackConfig, MediaConfig, AvcConfig, Mp4Sample};
...
pub fn decode_video_stream(app: &AppHandle, path: &str, label: String) -> Result<(), String> {
    ...
    let stream_index = ictx.streams()
        .best(Type::Video).ok_or("No video stream found")?.index();
    
    // 获取解码器
    let context_decoder = ffmpeg_next::codec::context::Context::new();
    let mut context = context_decoder.decoder().video().map_err(|e| e.to_string())?;
    ...
    mp4_writer.add_track(&track_config).map_err(|e| e.to_string())?;
    ...
    app.emit_to(EventTarget::WebviewWindow { label: label.clone() }, 
        "video-info", 
        video_info
    ).unwrap();
